...
<div class="contact popup">
    <div style="height: 600px" class="contact popup-box">
        ...
        <div class="popup-details"><h4 for="type"><b>Τύπος </b></h4><p id="type">Τηλεργασία</p></div>
...
<div style="margin: 0 auto; width: 800px;" class="menu_content wrapper hoc">
    ...
    $uId = $user['id'];
    if($user['employeeOf']) {
        $query = "select * from Suspension s, Company c where s.employee_id = '$uId' and s.company_id = c.id and s.type = 'telework' order by start_date DESC ";
        $susp_result = mysqli_query($conn, $query);
        if (!$susp_result) die($conn->error);
        if ($susp_result->num_rows == 0)
            echo '<h1  style="line-height:60px;text-align: center;text-transform: none; ">Δεν υπάρχει κάποια δήλωση τηλεργασίας από εργοδότη<br></h1>';
        else echo '<h1 style="text-align: center; text-transform: none">Οι δηλώσεις τηλεργασίας από τους εργοδότες σας</h1>';
        ...
        while ($row = mysqli_fetch_assoc($susp_result)) {
            ...
            echo "Δήλωση εξ' αποστάσεως εργασίας ";
        ...
        $query = "select s.id as susp_id, s.start_date,s.end_date,s.type,c.name,u.id,u.firstname,u.lastname from Suspension s, Company c, User u where c.employer_id = '$uId' and c.id = s.company_id and s.employee_id = u.id and s.type = 'telework' order by start_date DESC ";
        $result = mysqli_query($conn, $query);
        if (!$result) die($conn->error);
        if($result->num_rows == 0)
            echo '<h1  style="line-height:60px;text-align: center;text-transform: none; ">Δεν υπάρχει κάποια δήλωση τηλεργασίας για εργαζόμενο<br><a  href="EMPLOYERS/WORKFORCE/suspension-telework.php">Κάντε δήλωση τηλεργασίας για κάποιον εργαζόμενο σας</a></h1>';
        else echo '<h1 style="text-align: center; text-transform: none">Οι δηλώσεις τηλεργασίας για τους εργαζομένους σας</h1>';
        ...
        while($row = mysqli_fetch_assoc($result)){
            ...
            $en_dayM = $greek_months_srt[date('M', strtotime($row['end_date']))];
            $en_dayY = date('Y', strtotime($row['end_date']));
            echo "<a class='col-md-10' onclick='contactOpenDetails(\"".$greek_days[$st_dayName].' '.$st_day."\",\"".$greek_days[$en_dayName].' '.$en_day ."\",\"".$row['name']."\",\"".$row['firstname']."\",\"".$row['lastname']."\",\"".$row['id']."\")'>";
            ...
            if(strtotime(date('d-m-Y')) < strtotime(date('d-m-Y', strtotime($row['start_date'])))) echo '<footer style="display: inline; padding-bottom: 0" class="col-md-2 container row" >
                    ...
                    "><button onclick="return confirm(\'Θέλετε να ακυρώσετε αυτήν την δήλωση τηλεργασίας;\')" type="submit" name="delete"  style="color: black" class="btn btn-link" >ΑΚΥΡΩΣΗ</button>
                    </form></footer>';
...
<script>
    function contactOpenDetails (date1, date2, comp, fname, lname, afm) {
